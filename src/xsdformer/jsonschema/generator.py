...
class _JsonSchemaFromDesc:
  ...
  def generate(self, message_name: str) -> dict:
    ...
    Args:
        message_name: The fully qualified name of the message to generate the schema for.
  ...
  def _get_comment(self, desc) -> str | None:
    path = []
    file_name = ""
  ...
  def _find_descriptor_proto(self, desc: descriptor.Descriptor):
    fdp = self._fdp_map[desc.file.name]
    ...
    def find_nested(parent_dp, parts):
      if not parts:
        return parent_dp
      target_name = parts[0]
  ...
  def _find_enum_descriptor_proto(self, desc: descriptor.EnumDescriptor):
    fdp = self._fdp_map[desc.file.name]
    name_parts = desc.full_name.split(".")
    if fdp.package:
      name_parts = desc.full_name.replace(fdp.package + ".", "").split(".")

    def find_in_message(parent_dp, parts):
      if not parts:
        return None
      ...
      for nested_dp in parent_dp.nested_type:
        if nested_dp.name == target_name:
          return find_in_message(nested_dp, parts[1:])
      return None
    ...
    for dp in fdp.message_type:
      if dp.name == name_parts[0]:
        found = find_in_message(dp, name_parts[1:])
        if found:
          return found
    return None

  def _convert_message_to_schema(self, message_descriptor: descriptor.Descriptor) -> dict:
  ...
  def _convert_field_to_schema(self, field: descriptor.FieldDescriptor) -> dict:
    if field.is_repeated:
      if field.message_type and field.message_type.GetOptions().map_entry:
        ...
        schema = {"type": "array", "items": self._get_field_schema(field)}
    else:
  ...
  def _get_field_schema(self, field: descriptor.FieldDescriptor) -> dict:
    ...
    if field_type in [descriptor.FieldDescriptor.TYPE_DOUBLE, descriptor.FieldDescriptor.TYPE_FLOAT]:
      return {"type": "number"}
    if field_type in [
      ...
      return {"type": "integer"}
    if field_type == descriptor.FieldDescriptor.TYPE_BOOL:
      return {"type": "boolean"}
    if field_type == descriptor.FieldDescriptor.TYPE_STRING:
      return {"type": "string"}
    if field_type == descriptor.FieldDescriptor.TYPE_BYTES:
...
def generate(
  namespace: str, type_defs: tuple[xsd.TypeDefinition, ...], main_message: str
) -> str:
