...
def get_fields_occurs(
    ...
    match defn:
        case Enumeration():
            pass
        case Message():
            yield from itertools.chain.from_iterable(get_fields_occurs(c, occurs=occurs) for c in defn.content)
        case FieldContainer():
            yield from itertools.chain.from_iterable(
                get_fields_occurs(c, occurs=multiply_occurs(occurs, defn.occurs)) for c in defn.content
            )
        case Field():
...
def _include_type(xsd_type: xmlschema.XsdType | None) -> bool:
    ...
    def _is_enum(xsd_type: xmlschema.XsdType) -> bool:
        return bool(
            isinstance(xsd_type, xmlschema.validators.XsdAtomicRestriction) and xsd_type.enumeration is not None,
        )

    return not (xsd_type is None or (xsd_type.is_atomic() and not _is_enum(xsd_type)) or _is_any_type(xsd_type))
...
class _TypeRewriter:
    ...
    def rewrite(self, old_type: TypeDefinition, new_type: TypeDefinition) -> None:
        ...
        for t in self._type_to_fields:
            if isinstance(t, TypeDefinition) and t.enclosing_type and t.enclosing_type[0] is old_type:
